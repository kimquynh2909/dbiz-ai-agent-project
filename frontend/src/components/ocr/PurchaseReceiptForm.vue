<template>
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
    <div class="bg-white rounded-xl max-w-6xl w-full max-h-[95vh] overflow-hidden flex flex-col my-8">
      <div
        class="flex items-center justify-between p-6 border-b bg-gradient-to-r from-green-600 to-green-700 text-white"
      >
        <h3 class="text-xl font-bold">Kết quả OCR - Phiếu nhập</h3>
        ...
        <div v-if="ocrResult && ocrResult.extracted_data" class="space-y-6">
          ...
          <div class="bg-green-50 rounded-lg p-6 border border-green-200">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Số phiếu nhập</label>
                <input
                  ...
                  <tr v-for="(item, index) in formData.items" :key="index" class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-2 text-right">
                      <input
                        ...
                        type="number"
                        class="w-full px-2 py-1 border border-gray-300 rounded text-right focus:outline-none focus:ring-1 focus:ring-green-500"
                      />
                    </td>
                    <td class="px-4 py-2 text-right">
                      <input
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  type="number"
                  class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-white font-bold text-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                />
              </div>
              <div>
...
watch(
  ...
  (newResult) => {
    if (newResult && newResult.extracted_data) {
      ...
      formData.value.receipt_number = data.receipt_number || ''
      formData.value.receipt_date = data.receipt_date || ''
      formData.value.supplier = data.supplier || ''
...
const handleSave = async () => {
  try {
    ...
    const response = await call('dbiz_ai_agent.api.ocr.save_purchase_receipt', {
      receipt_data: formData.value,
